...
class ApiView(APIView):
    def post(self, request):
        ...
        for item in request.data['items']:
            ...
            if 'parentId' in item and item['parentId'] != None:
                ...
                try:
                    if not Main.objects.filter(id=uuid.UUID(parentId)).exists():
                        return Response({"code": 400, "message": "Validation Failed"}, status=400)
                    else:
                        parentId = Main.objects.filter(id=uuid.UUID(parentId))[0]
                        parent = parentId
                        while parent!=None:
            ...
            if 'price' in item:
                price = item['price']
            if Main.objects.filter(id=uuid.UUID(id)).exists():
                try:
                    obj = Main.objects.filter(id=uuid.UUID(id))[0]
                    obj.name = name
                    obj.type = type
                    if 'parentId' in item:
                ...
                try:
                    Main.objects.create(id=id, name=name, type=type, parentId=parentId, price=price, updateDate=updateDate)
                except:
                    return Response({"code": 400, "message": "Validation Failed"}, status=400)
    ...
    def delete(self, request, uid):
        try:
            if Main.objects.filter(id = uid).exists():
                obj = Main.objects.filter(id=uid)[0]
                obj.delete()
            else:
    ...
    def get(self,request):
        ...
        try:
            ...
            for item in query:
                ...
                ans['items'].append({'id':item.id, 'name':item.name, 'date':item.updateDate, 'parentId':parentID, 'price':item.price, 'type':item.type})
            return Response(ans)
        except:
            return Response({"status_code": 400, "detail": "Validation Failed"}, status=400)