    ...
    def post(self, request):
        ...
        for item in request.data['items']:
            ...
            if 'parentId' in item and item['parentId'] != None:
                ...
                try:
                    if not Main.objects.filter(uid=uuid.UUID(parentId)).exists():
                        return Response({"code": 400, "message": "Validation Failed"}, status=400)
                    else:
                        parentId = Main.objects.filter(uid=uuid.UUID(parentId))[0]
                        parent = parentId
                        while parent!=None:
            ...
            if 'price' in item:
                price = item['price']
            if Main.objects.filter(uid=uuid.UUID(id)).exists():
                try:
                    obj = Main.objects.filter(uid=uuid.UUID(id))[0]
                    obj.name = name
                    obj.type = type
                    if 'parentId' in item:
